    ...
    <div className="grid gap-5 lg:grid-cols-3">
      <Card className="lg:col-span-2">
        ...
        <form onSubmit={save} className="grid gap-4 md:grid-cols-2">
          ...
          <Button type="submit" icon={Save} className="md:col-span-2">
            حفظ الإعدادات
          </Button>
        </form>
      </Card>
      ...
      <Card>
        ...
        <div className="rounded-xl border border-ink-100 p-4">
          <div className="flex h-16 w-16 items-center justify-center rounded-xl bg-brand-600 text-2xl font-extrabold text-white">
            {settings.name?.charAt(0) || 'م'}
          </div>

          <h3 className="mt-4 font-extrabold text-ink-900">{settings.name}</h3>
          <p className="mt-1 text-sm font-bold text-brand-600">{settings.title}</p>
          <p className="mt-3 text-sm leading-6 text-ink-500">{settings.bio}</p>
        </div>
      </Card>
    </div>
  )
